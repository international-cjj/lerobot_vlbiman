...
EPISODE_TOGGLE_EVENT = "episode_toggle"
SAVE_EPISODE_EVENT = "save_episode"
DISCARD_EPISODE_EVENT = "discard_episode"
...
def init_keyboard_listener(*, manual_episode_control: bool = False):
    """
    Initializes a non-blocking keyboard listener for real-time user interaction.

    This function sets up a listener for specific keys (right arrow, left arrow, escape) to control
    the program flow during execution, such as stopping recording or exiting loops. When
    `manual_episode_control=True`, it also listens for space/enter/backspace to start-stop-save
    episodes. It gracefully
    ...
    events[EPISODE_TOGGLE_EVENT] = False
    events[SAVE_EPISODE_EVENT] = False
    events[DISCARD_EPISODE_EVENT] = False
    ...
    def on_press(key):
        try:
            if manual_episode_control and key == keyboard.Key.space:
                print("Space key pressed. Toggling current episode recording...")
                events[EPISODE_TOGGLE_EVENT] = True
            elif manual_episode_control and key == keyboard.Key.enter:
                print("Enter key pressed. Saving current episode...")
                events[SAVE_EPISODE_EVENT] = True
            elif manual_episode_control and key == keyboard.Key.backspace:
                print("Backspace key pressed. Discarding current episode...")
